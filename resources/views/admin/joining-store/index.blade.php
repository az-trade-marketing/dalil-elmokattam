...
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    ...
    function get_data() {
        $.ajax({
            ...
            type: 'GET',
            dataType: 'json',
            url: "/admin/joining-store-get-data",
    ...
    $.each(response, function(key, item) {
        ...
        // New columns
        var store_name = item.store_name ?? '';
        var owner_name = item.owner_name ?? '';
        var phone_number = item.phone_number ?? '';
        var zone_name = item.zone ? item.zone.name_en : ''; // Assuming zone relationship
        ...
        $('.country_table').append('<tr>\
            ...
            <td class="align-middle text-nowrap">\
                <h6 class="m-0 p-0">' + owner_name + '</h6>  \
            </td>\
            <td class="align-middle text-nowrap">\
                <h6 class="m-0 p-0">' + phone_number + '</h6>  \
            </td>\
            <td class="align-middle text-nowrap">\
                <h6 class="m-0 p-0">' + zone_name + '</h6>  \
            </td>\
            <td class="align-middle text-nowrap">\
    ...
    let table = new DataTable('#myTable');
        ...
        $(document).on('click', '.deleteButton', function() {
                ...
                $.ajax({
                    ...
                    type: 'get',
                    dataType: 'text',
                    url: "/admin/joining-store/" + item_id,
                    success: function(response) {
                        try {
